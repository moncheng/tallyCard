...
public class TallyCard implements Serializable {
	...
	public TallyCard(String name) {
		...
		this.count = 0;
		this.lastUpdateString = smartUpdateString();
	}
	...
	public String smartUpdateString() {
		...
		{
			smartString= Long.toString(d1=secsAgo/min)+trimS(d1," Minute ")+"and "+(d2=(secsAgo-(d1*min))/sec)+trimS(d2," Second ")+"ago.";
		}
		else if (secsAgo>=1*hr && secsAgo<23*hr)
		{
			smartString= Long.toString(d1=secsAgo/hr)+trimS(d1," Hour ")+"and "+(d2=(secsAgo-(d1*hr))/min)+trimS(d2," Minute ")+"ago.";
		}
		else if (secsAgo>=1*day && secsAgo<6*day)
		{
			smartString= Long.toString(d1=secsAgo/day)+trimS(d1," Day ")+"and "+(d2=(secsAgo-(d1*day))/hr)+trimS(d2," Hour ")+"ago.";
		}
		else if (secsAgo>=1*week && secsAgo<3*week)
		{
			smartString= Long.toString(d1=secsAgo/week)+trimS(d1," Week ")+"and "+(d2=(secsAgo-(d1*week))/day)+trimS(d2," Day ")+"ago.";
		}
		else if (secsAgo>=1*month && secsAgo<11*month)
		{
			smartString= Long.toString(d1=secsAgo/month)+trimS(d1," Month ")+"and "+(d2=(secsAgo-(d1*month))/week)+trimS(d2," Week ")+"ago.";
		}
		else if (secsAgo>=1*year)
		{
			smartString= Long.toString(d1=secsAgo/year)+trimS(d1," Year ")+"and "+(d2=(secsAgo-(d1*year))/month)+trimS(d2," Month ")+"ago.";
		}
		return smartString;
	}
